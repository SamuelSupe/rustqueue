...
RustQueue 0.8.2 aligns no-Channel retention with NSQ, qualifies the direct
Broker data path, and corrects premature Channel group commits. The comparison
bypasses Proxy and uses the production defaults; only the committed
qualification record may substantiate a performance claim, and it does not
generalize beyond the same OrbStack host used for the paired runs.
